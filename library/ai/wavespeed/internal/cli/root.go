...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "wavespeed-pp-cli",
		Short: "Manage wavespeed resources via the wavespeed API",
		Long: `Manage wavespeed resources via the wavespeed API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'wavespeed-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("wavespeed-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'wavespeed-pp-cli profile list')")
	...
	rootCmd.AddCommand(newBillingsPromotedCmd(flags))
	rootCmd.AddCommand(newMediaUploadsPromotedCmd(flags))
	rootCmd.AddCommand(newModelPricingPromotedCmd(flags))
	rootCmd.AddCommand(newModelsPromotedCmd(flags))
	rootCmd.AddCommand(newPredictionDeletionsPromotedCmd(flags))
	rootCmd.AddCommand(newPredictionResultsPromotedCmd(flags))
	rootCmd.AddCommand(newPredictionsPromotedCmd(flags))
	rootCmd.AddCommand(newUsageStatsPromotedCmd(flags))
