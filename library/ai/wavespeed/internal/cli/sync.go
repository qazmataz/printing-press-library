...
Resource scoping:
  ...
  wavespeed-pp-cli sync

  # Sync specific resources only
  wavespeed-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  wavespeed-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  wavespeed-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  wavespeed-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  wavespeed-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "account_balance":
		return db.UpsertAccountBalance(data)
	case "billings":
		return db.UpsertBillings(data)
	case "media_uploads":
		return db.UpsertMediaUploads(data)
	case "model_pricing":
		return db.UpsertModelPricing(data)
	case "models":
		return db.UpsertModels(data)
	case "prediction_deletions":
		return db.UpsertPredictionDeletions(data)
	case "prediction_results":
		return db.UpsertPredictionResults(data)
	case "predictions":
		return db.UpsertPredictions(data)
	case "usage_stats":
		return db.UpsertUsageStats(data)
	default:
