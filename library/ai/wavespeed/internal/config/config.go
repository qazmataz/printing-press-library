...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("WAVESPEED_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "wavespeed-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("WAVESPEED_API_KEY"); v != "" {
		cfg.WavespeedApiKey = v
		cfg.AuthSource = "env:WAVESPEED_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.WavespeedApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("WAVESPEED_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.WavespeedApiKey
	if token == "" {
		return ""
	}
	if c.WavespeedApiKey == "" {
		return ""
	}
	replacements := map[string]string{
		"token":             token,
		"api_key":           c.WavespeedApiKey,
		"WAVESPEED_API_KEY": c.WavespeedApiKey,
	}
