...
type Config struct {
	...
	Path           string            `toml:"-"`
	ClockifyApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("CLOCKIFY_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "clockify-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("CLOCKIFY_API_KEY"); v != "" {
		cfg.ClockifyApiKey = v
		cfg.AuthSource = "env:CLOCKIFY_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.ClockifyApiKey != "" {
		cfg.AuthSource = "config"
	}
