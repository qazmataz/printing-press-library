...
// to ~/.clockify-pp-cli/jobs.jsonl; the latest row for a given JobID wins
...
~/.clockify-pp-cli/jobs.jsonl. This command lists, inspects, and prunes them.
...
func newJobsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  # List the latest row per job
  clockify-pp-cli jobs list

  # Every state transition, as JSON
  clockify-pp-cli jobs list --all --json`,
...
func newJobsGetCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example:     "  clockify-pp-cli jobs get job-3f9a2c1e --json",
		Annotations: map[string]string{"mcp:read-only": "true"},
...
func newJobsPruneCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  # Prune job ledger rows older than 7 days
  clockify-pp-cli jobs prune --older-than 168h`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			rows, err := readJobRows()
			if err != nil {
				return err
