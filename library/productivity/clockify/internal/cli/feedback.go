...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".clockify-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CLOCKIFY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CLOCKIFY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "clockify-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.clockify-pp-cli/feedback.jsonl.
When ` + "`CLOCKIFY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CLOCKIFY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  clockify-pp-cli feedback list
  clockify-pp-cli feedback list --limit 5
  clockify-pp-cli feedback list --json`,
