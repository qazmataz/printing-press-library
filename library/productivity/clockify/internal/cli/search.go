...
In local mode: searches locally synced data only.`,
  ...
  clockify-pp-cli search "error timeout"

  # Force local search only
  clockify-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  clockify-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  clockify-pp-cli search "critical" --json --limit 20`,
		Annotations: map[string]string{"mcp:hidden": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
			// Quote FTS5 special characters so queries like "pp-test" do not
			// raise a SQL error (hand patch — see novel_helpers.go).
			query := ftsSanitizeQuery(args[0])
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'clockify-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "custom-fields":
				results, err = db.SearchCustomFields(query, limit)
			case "invoices":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchCustomFields(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search custom_fields failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
