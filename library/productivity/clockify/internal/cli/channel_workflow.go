...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  clockify-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  clockify-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.asJSON {
				// syncResource streams NDJSON {"event":...} lines to stdout;
				// emit the summary as one more NDJSON line so the whole
				// --json stream stays a consistent newline-delimited stream.
				// A pretty-printed object here would make stdout unparseable.
				fmt.Fprintf(cmd.OutOrStdout(),
					`{"event":"archive_complete","resources_synced":%d,"total_items":%d,"store_path":%q,"timestamp":%q}`+"\n",
					len(resources), totalSynced, dbPath, time.Now().UTC().Format(time.RFC3339))
				return nil
			}
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  clockify-pp-cli workflow status

  # Show status as JSON
  clockify-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("clockify-pp-cli")
			}
