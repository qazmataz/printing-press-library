...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "clockify-pp-cli",
		Short: `Clockify CLI — Every Clockify feature, plus a local time database that reconstructs your weekly timesheet, finds untracked gaps, and a…`,
		Long: `Clockify CLI — Every Clockify feature, plus a local time database that reconstructs your weekly timesheet, finds untracked gaps, and a…
...
Health check: run 'clockify-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("clockify-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'clockify-pp-cli profile list')")
	...
	rootCmd.AddCommand(newApprovalRequestsCmd(flags))
	...
	rootCmd.AddCommand(newMemberProfileCmd(flags))
	projectsCmd := newProjectsCmd(flags)
	projectsCmd.AddCommand(newProjectBurnCmd(flags)) // transcendence: budget burn
	...
	rootCmd.AddCommand(newTimeOffCmd(flags))
	...
	rootCmd.AddCommand(newCostRatePromotedCmd(flags))
	rootCmd.AddCommand(newFilePromotedCmd(flags))
	rootCmd.AddCommand(newHourlyRatePromotedCmd(flags))
	rootCmd.AddCommand(newUserPromotedCmd(flags))
	// Transcendence features (hand-built, see absorb manifest).
	...
	rootCmd.AddCommand(newBillableCmd(flags))
