# Clockify Printed CLI Agent Guide

This directory is a generated `clockify-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
clockify-pp-cli doctor --json
clockify-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
clockify-pp-cli which "<capability>" --json
...
```bash
clockify-pp-cli <command> --help
clockify-pp-cli <command> --dry-run --agent
```
