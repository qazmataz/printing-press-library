...
// PATCH config-base-url: hand-set BaseURL default to https://api.fireflies.ai (generator emits a placeholder).
package config
...
type Config struct {
	...
	Path           string `toml:"-"`
	FirefliesApiKey string `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("FIREFLIES_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "fireflies-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("FIREFLIES_API_KEY"); v != "" {
		cfg.FirefliesApiKey = v
		cfg.AuthSource = "env:FIREFLIES_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.FirefliesApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("FIREFLIES_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.FirefliesApiKey == "" {
		return ""
	}
	return "Bearer " + c.FirefliesApiKey
}
