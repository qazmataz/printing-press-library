...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  strava-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  strava-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"athlete", "athlete-activities", "segments"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  strava-pp-cli workflow status

  # Show status as JSON
  strava-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("strava-pp-cli")
			}
