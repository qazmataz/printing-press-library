...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "strava-pp-cli",
		Short: `Strava CLI — Every Strava feature, plus offline analytics — training load, power curves, zone time, and segment progression — that t…`,
		Long: `Strava CLI — Every Strava feature, plus offline analytics — training load, power curves, zone time, and segment progression — that t…
...
Health check: run 'strava-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("strava-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'strava-pp-cli profile list')")
	...
	rootCmd.AddCommand(newAthleteCmd(flags))
	rootCmd.AddCommand(newAthletesCmd(flags))
	rootCmd.AddCommand(newSegmentEffortsCmd(flags))
	...
	rootCmd.AddCommand(newClubsPromotedCmd(flags))
	rootCmd.AddCommand(newGearPromotedCmd(flags))
	rootCmd.AddCommand(newRoutesPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())
	rootCmd.AddCommand(newTrainingCmd(flags))
