...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".strava-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("STRAVA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("STRAVA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "strava-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.strava-pp-cli/feedback.jsonl.
When ` + "`STRAVA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`STRAVA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  strava-pp-cli feedback list
  strava-pp-cli feedback list --limit 5
  strava-pp-cli feedback list --json`,
