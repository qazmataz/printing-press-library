...
Resource scoping:
  ...
  strava-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  strava-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  strava-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  strava-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  strava-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/strava-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "kudos":
		return db.UpsertKudos(data)
	case "laps":
		return db.UpsertLaps(data)
	case "activities_streams":
		return db.UpsertActivitiesStreams(data)
	case "zones":
		return db.UpsertZones(data)
	case "athletes_routes":
		return db.UpsertAthletesRoutes(data)
	case "stats":
		return db.UpsertStats(data)
	case "clubs_activities":
		return db.UpsertClubsActivities(data)
	case "admins":
		return db.UpsertAdmins(data)
	case "members":
		return db.UpsertMembers(data)
	case "export_gpx":
		return db.UpsertExportGpx(data)
	case "export_tcx":
		return db.UpsertExportTcx(data)
	case "routes_streams":
		return db.UpsertRoutesStreams(data)
	case "segment_efforts_streams":
		return db.UpsertSegmentEffortsStreams(data)
	case "starred":
		return db.UpsertStarred(data)
	case "segments_streams":
		return db.UpsertSegmentsStreams(data)
	default:
