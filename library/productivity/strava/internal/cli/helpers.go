...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			"\n      Set your API key: export STRAVA_ACCESS_TOKEN=<your-key>"+
			"\n      Run 'strava-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: strava-pp-cli auth set-token <token>"+
			"\n      or: export STRAVA_ACCESS_TOKEN=<your-token>"+
			"\n      Run 'strava-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. Your credentials are valid but lack access to this resource."+
			"\n      Check that your API key has the required permissions."+
			"\n      Set it with: export STRAVA_ACCESS_TOKEN=<your-key>"+
			"\n      Run 'strava-pp-cli doctor' to check auth status.", err))
...
// jsonFloat extracts a float64 from a map[string]any, tolerating JSON-encoded strings.
...
// extractStreamValues extracts a float64 slice from a Strava stream JSON blob.
// Stream blobs may be {"data":[...], ...} or a top-level map keyed by stream type.
func extractStreamValues(streamJSON string, key string) []float64 {
