...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Strava",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  strava-pp-cli auth setup\n  strava-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "No setup URL is configured for this CLI; check the API's docs.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then run:")
			fmt.Fprintln(w, "  strava-pp-cli auth login --client-id <id> --client-secret <secret>")
			if !launch {
				return nil
			}
...
func newAuthLoginCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			scopes := []string{"activity:read", "activity:read_all", "activity:write", "profile:read_all", "profile:write", "read", "read_all"}
			if len(scopes) > 0 {
	...
	cmd.Flags().StringVar(&clientID, "client-id", os.Getenv("STRAVA_CLIENT_ID"), "OAuth2 client ID")
	cmd.Flags().StringVar(&clientSecret, "client-secret", os.Getenv("STRAVA_CLIENT_SECRET"), "OAuth2 client secret")
