...
type Config struct {
	...
	Path              string `toml:"-"`
	StravaStravaOauth string `toml:"strava_oauth"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("STRAVA_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "strava-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides. Accept both the canonical STRAVA_ACCESS_TOKEN and the
	// spec-slug-derived STRAVA_STRAVA_OAUTH for backwards compatibility.
	if v := os.Getenv("STRAVA_ACCESS_TOKEN"); v != "" {
		cfg.StravaStravaOauth = v
		cfg.AuthSource = "env:STRAVA_ACCESS_TOKEN"
	} else if v := os.Getenv("STRAVA_STRAVA_OAUTH"); v != "" {
		cfg.StravaStravaOauth = v
		cfg.AuthSource = "env:STRAVA_STRAVA_OAUTH"
	}
	...
	if cfg.AuthSource == "" && cfg.StravaStravaOauth != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("STRAVA_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("STRAVA_AUTHORIZATION_URL"); v != "" {
...
func (c *Config) AuthHeader() string {
	...
	if c.StravaStravaOauth != "" {
		c.AuthSource = "env:STRAVA_STRAVA_OAUTH"
		return "Bearer " + c.StravaStravaOauth
	}
	if c.AccessToken != "" {
		c.AuthSource = "oauth2"
