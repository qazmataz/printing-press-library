# Strava Printed CLI Agent Guide

This directory is a generated `strava-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
strava-pp-cli doctor --json
strava-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
strava-pp-cli which "<capability>" --json
strava-pp-cli <command> --help
```
...
```bash
strava-pp-cli <command> --help
strava-pp-cli <command> --dry-run --agent
```
