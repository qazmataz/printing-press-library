# Safari History Gen Printed CLI Agent Guide

This directory is a generated `safari-history-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
safari-history-pp-cli doctor --json
safari-history-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
safari-history-pp-cli which "<capability>" --json
safari-history-pp-cli <command> --help
```
...
```bash
safari-history-pp-cli <command> --agent
```
...
```bash
safari-history-pp-cli <command> --help
safari-history-pp-cli <command> --dry-run --agent
```
