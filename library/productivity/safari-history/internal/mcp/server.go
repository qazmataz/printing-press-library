...
func tools() []toolSpec {
	return []toolSpec{
		mk("search", "FTS search over URL/title", []arg{{"query", true, "Search query"}, {"domain", false, "Domain filter"}, {"device", false, "Device filter"}, {"since", false, "Since window"}, {"limit", false, "Row limit"}}, func(r mcp.CallToolRequest) []string {
			args := []string{"search"}
		...
		mk("journeys", "Journeys cluster listing", []arg{{"since", false, "Since window"}, {"limit", false, "Row limit"}}, func(r mcp.CallToolRequest) []string {
