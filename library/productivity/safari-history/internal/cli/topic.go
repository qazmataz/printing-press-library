...
func newTopicCmd(opts *RootOptions) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "topic <name>",
		Short:       "Gather everything you browsed about a topic over --since via full-text page matches (merges Journeys clusters too when the source has them)",
		Args:        usageMinArgs(1),
		Example:     strings.Trim("safari-history-pp-cli topic fountain pens --since 30d --limit 50", "\n"),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fromFTS, err := opts.Source.FullTextSearch(st.DB(), name, source.VisitFilter{Limit: opts.Output.Limit, Since: start, Device: opts.Device})
			if err != nil {
				return err
			}
