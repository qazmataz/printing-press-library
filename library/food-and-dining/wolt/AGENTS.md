# Wolt Printed CLI Agent Guide

This directory is a generated `wolt-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
wolt-pp-cli doctor --json
wolt-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
wolt-pp-cli which "<capability>" --json
...
```bash
wolt-pp-cli <command> --help
wolt-pp-cli <command> --dry-run --agent
```
