...
// TestMigrate_AddsColumnsOnUpgrade_OrderXp verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_OrderXp(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "order_xp" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	rows, err := s.DB().Query(`PRAGMA table_info("order_xp")`)
	if err != nil {
	...
	for _, want := range []string{
		"do_use_backend_pricing",
		"is_venue_favourite",
	} {
		if !hasColumn[want] {
			t.Fatalf("%s column missing from order_xp after migrate", want)
		}
	}
}
