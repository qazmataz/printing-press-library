...
In local mode: searches locally synced data only.`,
  ...
  wolt-pp-cli search "error timeout"

  # Force local search only
  wolt-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  wolt-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  wolt-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /v1/pages/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/v1/pages/search", map[string]any{
					"q":      query,
					"lat":    nil,
					"lon":    nil,
					"target": "venues",
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'wolt-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/wolt-pp-cli/data.db)")
