...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "wolt-pp-cli",
		Short: "Browse-only Wolt CLI for city discovery, venue search, and per-venue ETA snapshots",
		Long: `Browse-only Wolt consumer CLI: list cities, find restaurants near a lat/lon,
search venues and items, and pull per-venue delivery/ETA snapshots — all via
Wolt's unauthenticated consumer endpoints.

Add --agent to any command for JSON output + non-interactive mode.
Run 'wolt-pp-cli doctor' to verify connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("wolt-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'wolt-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCitiesPromotedCmd(flags))
	rootCmd.AddCommand(newOrderXpPromotedCmd(flags))
	rootCmd.AddCommand(newVenuesPromotedCmd(flags))
	rootCmd.AddCommand(newVenuesNowCmd(flags))
	rootCmd.AddCommand(newVenuesCompareCmd(flags))
	rootCmd.AddCommand(newCuisineBottleneckCmd(flags))
