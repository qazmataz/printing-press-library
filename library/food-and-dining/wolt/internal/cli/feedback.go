...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".wolt-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("WOLT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("WOLT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "wolt-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.wolt-pp-cli/feedback.jsonl.
When ` + "`WOLT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`WOLT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  wolt-pp-cli feedback list
  wolt-pp-cli feedback list --limit 5
  wolt-pp-cli feedback list --json`,
