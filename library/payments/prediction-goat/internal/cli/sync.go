...
Resource scoping:
  ...
  prediction-goat-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  prediction-goat-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  prediction-goat-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  prediction-goat-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  prediction-goat-pp-cli sync --latest-only`,
...
func resourceSupportsPagination(resource string) bool {
	...
	case "events":
		return true
	case "events-creators":
		return true
	case "events-keyset":
		return true
	case "events-pagination":
		return true
	case "events-results":
		return true
	case "markets":
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "events_tags":
		return db.UpsertEventsTags(data)
	case "tweet_count":
		return db.UpsertTweetCount(data)
	case "markets":
		return db.UpsertMarkets(data)
	case "description":
		return db.UpsertDescription(data)
	case "markets_tags":
		return db.UpsertMarketsTags(data)
	case "profiles":
		return db.UpsertProfiles(data)
	case "public-profile":
		return db.UpsertPublicProfile(data)
	case "series":
		return db.UpsertSeries(data)
	case "series_comments":
		return db.UpsertSeriesComments(data)
	case "series-summary":
		return db.UpsertSeriesSummary(data)
	case "sports":
		return db.UpsertSports(data)
	case "tags":
		return db.UpsertTags(data)
	case "related_tags":
		return db.UpsertRelatedTags(data)
	case "teams":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"comments":            "/comments",
		"events":              "/events",
		"events-creators":     "/events/creators",
		"events-keyset":       "/events/keyset",
		"events-pagination":   "/events/pagination",
		"events-results":      "/events/results",
		"markets":             "/markets",
		"markets-keyset":      "/markets/keyset",
		"series":              "/series",
		"sports":              "/sports",
		"sports-market-types": "/sports/market-types",
		"tags":                "/tags",
		"teams":               "/teams",
	}
