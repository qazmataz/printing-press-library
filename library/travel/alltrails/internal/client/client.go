...
import (
	...
	"errors"
	"fmt"
	"github.com/mvanhorn/printing-press-library/library/travel/alltrails/internal/cliutil"
...
const AllowWritesEnv = "ALLTRAILS_PP_ALLOW_WRITES"
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "alltrails-pp-cli", "http")
...
func liveWritesAllowed() bool {
	return os.Getenv(AllowWritesEnv) == "1"
}

func liveWriteBlockedError(method, path string) error {
	return fmt.Errorf("live AllTrails write blocked for %s %s: rerun with --dry-run to preview, or export %s=1 only after explicit approval", method, path, AllowWritesEnv)
}
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	verifyLiveHTTP := cliutil.IsVerifyEnv() && cliutil.IsVerifyLiveHTTPEnv()
	...
	if !readOnlyIntent && isMutatingVerb(method) && cliutil.IsVerifyEnv() && !verifyLiveHTTP {
		return verifyShortCircuitEnvelope(method, path), http.StatusOK, nil
	}
	if !readOnlyIntent && isMutatingVerb(method) && !c.DryRun && !verifyLiveHTTP && !liveWritesAllowed() {
		return nil, 0, liveWriteBlockedError(method, path)
	}
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "alltrails-pp-cli/0.1.0")
		}
