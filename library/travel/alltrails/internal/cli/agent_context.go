...
func buildAgentContext(rootCmd *cobra.Command) agentContext {
	envVars := []agentContextAuthEnvVar{
		{
			Name:        "ALLTRAILS_ACCESS_TOKEN",
			Kind:        "per_call",
			Required:    false,
			Sensitive:   true,
			Description: "Bearer token for authenticated AllTrails API requests.",
		},
		{
			Name:        "ALLTRAILS_COOKIE",
			Kind:        "per_call",
			Required:    false,
			Sensitive:   true,
			Description: "Browser session Cookie header for authenticated AllTrails routes.",
		},
		{
			Name:        "ALLTRAILS_CSRF_TOKEN",
			Kind:        "per_call",
			Required:    false,
			Sensitive:   true,
			Description: "CSRF token header used by browser-backed AllTrails write routes when required.",
		},
		{
			Name:        "ALLTRAILS_PP_ALLOW_WRITES",
			Kind:        "approval_gate",
			Required:    false,
			Sensitive:   false,
			Description: "Set to 1 only after explicit approval to allow live AllTrails write requests.",
		},
	}
