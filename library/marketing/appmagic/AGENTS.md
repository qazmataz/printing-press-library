# AppMagic Printed CLI Agent Guide

This directory is a generated `appmagic-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
appmagic-pp-cli doctor --json
appmagic-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
appmagic-pp-cli which "<capability>" --json
appmagic-pp-cli <command> --help
```
...
```bash
appmagic-pp-cli <command> --help
appmagic-pp-cli <command> --dry-run --agent
```
