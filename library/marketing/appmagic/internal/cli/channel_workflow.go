...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  appmagic-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  appmagic-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"adint", "app-categories", "app-publishers", "app-tags", "app-transfers", "apps-downloads", "categories", "country-downloads", "last-date", "last-versions", "live-ops-live-ops", "live-ops-live-ops-calendar", "live-ops-tags-values", "live-ops-updates", "period-comparison", "sdkint", "steam", "steam-app-united", "steam-categories", "steam-genres", "tags", "tops"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  appmagic-pp-cli workflow status

  # Show status as JSON
  appmagic-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("appmagic-pp-cli")
			}
