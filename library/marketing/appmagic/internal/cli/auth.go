...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Appmagic",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  appmagic-pp-cli auth setup\n  appmagic-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "No setup URL is configured for this CLI; check the API's docs.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export APPMAGIC_LOGIN=\"<your-login>\"")
			fmt.Fprintln(w, "  export APPMAGIC_PASSWORD=\"<your-password>\"")
			fmt.Fprintln(w, "Both variables are required: the API uses HTTP Basic auth with your AppMagic account credentials.")
			if !launch {
				return nil
			}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  appmagic-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export APPMAGIC_LOGIN=\"your-token-here\"")
				fmt.Fprintln(w, "  export APPMAGIC_PASSWORD=\"your-token-here\"")
				fmt.Fprintf(w, "  appmagic-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  appmagic-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  appmagic-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("APPMAGIC_LOGIN") != "" {
				envStillSet = "APPMAGIC_LOGIN"
			}
			if envStillSet == "" && os.Getenv("APPMAGIC_PASSWORD") != "" {
				envStillSet = "APPMAGIC_PASSWORD"
			}
