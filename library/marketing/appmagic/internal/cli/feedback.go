...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "appmagic-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("APPMAGIC_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("APPMAGIC_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "appmagic-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/appmagic-pp-cli/feedback.jsonl.
When ` + "`APPMAGIC_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`APPMAGIC_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  appmagic-pp-cli feedback list
  appmagic-pp-cli feedback list --limit 5
  appmagic-pp-cli feedback list --json`,
