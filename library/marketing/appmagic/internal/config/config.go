...
type Config struct {
	...
	AppmagicLogin    string            `toml:"login"`
	AppmagicPassword string            `toml:"password"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.appmagic.rocks/v1",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("APPMAGIC_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "appmagic-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("APPMAGIC_LOGIN"); v != "" {
		cfg.AppmagicLogin = v
		cfg.markEnvOverride("AppmagicLogin")
		cfg.AuthSource = "env:APPMAGIC_LOGIN"
	}
	if v := os.Getenv("APPMAGIC_PASSWORD"); v != "" {
		cfg.AppmagicPassword = v
		cfg.markEnvOverride("AppmagicPassword")
		cfg.AuthSource = "env:APPMAGIC_PASSWORD"
	}
	...
	if cfg.AuthSource == "" && cfg.AppmagicLogin != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.AppmagicPassword != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("APPMAGIC_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.AppmagicLogin == "" || c.AppmagicPassword == "" {
		return ""
	}
	credentials := c.AppmagicLogin + ":" + c.AppmagicPassword
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.AppmagicLogin = token
	delete(c.envOverrides, "AppmagicLogin")
	c.updateFileConfigField("AppmagicLogin")
	return c.save()
}
...
func (c *Config) ClearTokens() error {
	...
	delete(c.envOverrides, "AppmagicLogin")
	c.AppmagicPassword = ""
	delete(c.envOverrides, "AppmagicPassword")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["AppmagicLogin"] {
			out.AppmagicLogin = c.fileConfig.AppmagicLogin
		}
		if c.envOverrides["AppmagicPassword"] {
			out.AppmagicPassword = c.fileConfig.AppmagicPassword
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "AppmagicLogin":
		c.fileConfig.AppmagicLogin = c.AppmagicLogin
	case "AppmagicPassword":
		c.fileConfig.AppmagicPassword = c.AppmagicPassword
	}
}
