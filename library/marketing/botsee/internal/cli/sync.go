...
Resource scoping:
  ...
  botsee-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  botsee-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  botsee-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  botsee-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  botsee-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/botsee-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "keyword_opportunities":
		return db.UpsertKeywordOpportunities(data)
	case "keywords":
	...
	case "source_opportunities":
		return db.UpsertSourceOpportunities(data)
	case "sources":
	...
	case "customer_types_personas":
		return db.UpsertCustomerTypesPersonas(data)
	case "personas_questions":
		return db.UpsertPersonasQuestions(data)
	case "results":
		return db.UpsertResults(data)
	case "rate-limits":
		return db.UpsertRateLimits(data)
	case "signup":
		return db.UpsertSignup(data)
	case "pay_usdc":
		return db.UpsertPayUsdc(data)
	case "status":
		return db.UpsertStatus(data)
	case "sites_analysis":
		return db.UpsertSitesAnalysis(data)
	case "sites_customer_types":
		return db.UpsertSitesCustomerTypes(data)
	case "usage":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"billing":         "/api/v1/billing/settings",
		"pricing":         "/api/v1/pricing",
		"rate-limits":     "/api/v1/rate-limits",
		"sites":           "/api/v1/sites",
		"usage":           "/api/v1/usage",
		"usage-by-key":    "/api/v1/usage/by-key",
		"webhooks":        "/api/v1/webhooks",
		"webhooks-events": "/api/v1/webhooks/events",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "sites_analysis", ParentTable: "sites", ParentIDParam: "site_uuid", PathTemplate: "/api/v1/sites/{site_uuid}/analysis", KeyField: ""},
	}
}
