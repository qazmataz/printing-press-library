...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".botsee-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BOTSEE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BOTSEE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "botsee-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.botsee-pp-cli/feedback.jsonl.
When ` + "`BOTSEE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BOTSEE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  botsee-pp-cli feedback list
  botsee-pp-cli feedback list --limit 5
  botsee-pp-cli feedback list --json`,
