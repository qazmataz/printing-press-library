...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  botsee-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  botsee-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"account", "api-keys", "billing", "pricing", "rate-limits", "sites", "usage", "usage-by-key", "webhooks", "webhooks-events"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  botsee-pp-cli workflow status

  # Show status as JSON
  botsee-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("botsee-pp-cli")
			}
