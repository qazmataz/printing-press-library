# BotSee Printed CLI Agent Guide

This directory is a generated `botsee-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
botsee-pp-cli doctor --json
botsee-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
botsee-pp-cli which "<capability>" --json
botsee-pp-cli <command> --help
```
...
```bash
botsee-pp-cli <command> --help
botsee-pp-cli <command> --dry-run --agent
```
