...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
  ...
  plane-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if os.Getenv("PLANE_API_KEY_AUTHENTICATION") != "" {
				authEnvSet = append(authEnvSet, "PLANE_API_KEY_AUTHENTICATION")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "PLANE_API_KEY_AUTHENTICATION")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'plane-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'plane-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
