...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  plane-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  plane-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"invitations", "members", "projects", "stickies"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  plane-pp-cli workflow status

  # Show status as JSON
  plane-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("plane-pp-cli")
			}
