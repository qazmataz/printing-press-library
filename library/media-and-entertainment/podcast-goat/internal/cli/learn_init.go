...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		dbPath := defaultDBPath("podcast-goat-pp-cli")
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: podcast-goat-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: podcast-goat-pp-cli: learn init: %v\n", err)
		}
	})
}
