// Copyright 2026 Podcast GOAT. Licensed under Apache-2.0. See LICENSE.
// Generated by CLI Printing Press (https://github.com/mvanhorn/cli-printing-press). DO NOT EDIT.

// teach.go implements the LLM-driven learning surface for the
// podcast-goat-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/podcast-goat/internal/learn"
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/podcast-goat/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/podcast-goat/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/podcast-goat/internal/learn/patterns"
...
const noLearnEnvVar = "PODCAST_GOAT_NO_LEARN"
...
func learningsStateDir() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "podcast-goat-pp-cli")
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("podcast-goat-pp-cli")
}
...
~/.local/share/podcast-goat-pp-cli/teach.log, safe to fire-and-forget.

Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  podcast-goat-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  podcast-goat-pp-cli recall "<question>" --agent
  podcast-goat-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  podcast-goat-pp-cli learnings list --agent
  podcast-goat-pp-cli learnings list --query "<substring>"
  podcast-goat-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if learnDryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  podcast-goat-pp-cli learnings forget "<question>" --resource <id>
  podcast-goat-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
into a whole family.`,
		Example: `  podcast-goat-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  podcast-goat-pp-cli teach-lookup --kind country --canonical "United States" --alias USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if learnDryRunOK(flags) {
				return nil
			}
