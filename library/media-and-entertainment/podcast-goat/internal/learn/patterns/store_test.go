...
// openTestDB stands up the v3 search_patterns schema only — no other
// tables. The pattern store doesn't depend on resources / learnings;
// keeping the fixture minimal lets these tests stay fast.
func openTestDB(t *testing.T) *sql.DB {
	...
	`); err != nil {
		t.Fatalf("create search_patterns: %v", err)
	}
	return db
}
...
func TestUpsert_InsertsNewRow(t *testing.T) {
	t.Parallel()
	db := openTestDB(t)
	inserted, err := Upsert(db, samplePattern())
	if err != nil {
...
func TestUpsert_DuplicateBumpsConfidence(t *testing.T) {
	...
	db := openTestDB(t)
	r := samplePattern()
	if _, err := Upsert(db, r); err != nil {
		t.Fatalf("first: %v", err)
	}
	inserted, err := Upsert(db, r)
	if err != nil {
...
func TestUpsert_RejectsMissingRequiredFields(t *testing.T) {
	...
	cases := []struct {
		...
		{"empty query_template", func(r *Pattern) { r.QueryTemplate = "" }},
		{"empty resource_template", func(r *Pattern) { r.ResourceTemplate = "" }},
		{"empty strategy", func(r *Pattern) { r.Strategy = "" }},
		{"empty entity_kind", func(r *Pattern) { r.EntityKind = "" }},
		{"empty resource_type", func(r *Pattern) { r.ResourceType = "" }},
	}
...
func TestList_FiltersByEntityKindAndStrategy(t *testing.T) {
	...
	if _, err := Upsert(db, r1); err != nil {
		t.Fatalf("upsert r1: %v", err)
	}
	if _, err := Upsert(db, r2); err != nil {
...
func TestForget_AllWipesEverything(t *testing.T) {
	t.Parallel()
	db := openTestDB(t)
	if _, err := Upsert(db, samplePattern()); err != nil {
		t.Fatalf("upsert: %v", err)
	}
