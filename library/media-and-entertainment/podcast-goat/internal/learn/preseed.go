...
// the same (query_pattern, resource_type); preseed is a pure-insert
// operation, never a confidence bump.
func insertOne(ctx context.Context, db *sql.DB, row PreseedRow) (bool, error) {
	...
	// Existence check: any row at the (query_pattern, resource_type)
	// PK blocks the insert. Preseed never overwrites or bumps; the
	// recall path will read whatever is already there.
	var existing string
	err := db.QueryRowContext(ctx,
		`SELECT resource_ids FROM search_learnings WHERE query_pattern = ? AND resource_type = ?`,
		row.QueryPattern, row.ResourceType,
	).Scan(&existing)
	if err == nil {
		return false, nil
	}
	...
	idsJSON, err := json.Marshal([]string{row.ResourceID})
	if err != nil {
		return false, fmt.Errorf("preseed marshal ids: %w", err)
	}
	...
	_, err = db.ExecContext(ctx,
		`INSERT INTO search_learnings
		 ...
		 VALUES (?, ?, ?, ?, ?, 'boost', ?, ?, ?, ?)`,
		row.QueryPattern, string(entitiesJSON), string(idsJSON), row.ResourceType, row.Venue,
		preseedConfidenceFloor, source, now, now,
	)
	if err != nil {
