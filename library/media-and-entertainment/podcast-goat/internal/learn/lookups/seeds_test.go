...
func TestSeedFromConfig_InsertsCanonicalAndAliases(t *testing.T) {
	t.Parallel()
	db := openTestDB(t)
	seeds := map[string][]SeedConfig{
		"team_code": {
			{Canonical: "Alpha", Aliases: []string{"ALP", "Alpha United"}},
			{Canonical: "Bravo", Aliases: []string{"BRV"}},
		},
	}
	...
	// Expect 5 rows: 2 canonicals + 3 aliases.
	if inserted != 5 {
		t.Errorf("inserted = %d, want 5", inserted)
	}
	// Canonical lookups should return the canonical itself.
	got, ok, err := Lookup(db, "team_code", "Alpha")
	if err != nil || !ok {
		t.Fatalf("Lookup Alpha: ok=%v err=%v", ok, err)
	}
	// Source-priority sort puts whichever row landed first ("Alpha" or "ALP" or "Alpha United") at the top.
	// All three are valid aliases — just confirm we got something non-empty.
	if got == "" {
		t.Errorf("Lookup Alpha returned empty")
	}
	// LookupAll must surface canonical + both aliases.
...
func TestSeedFromConfig_Idempotent(t *testing.T) {
	...
	db := openTestDB(t)
	seeds := map[string][]SeedConfig{
		"team_code": {{Canonical: "Alpha", Aliases: []string{"ALP"}}},
	}
...
func TestSeedFromConfig_CanonicalAsItsOwnAlias(t *testing.T) {
	t.Parallel()
	db := openTestDB(t)
	// Author lists the canonical explicitly in Aliases; UNIQUE constraint
	// should silence the duplicate.
	seeds := map[string][]SeedConfig{
		"team_code": {{Canonical: "Alpha", Aliases: []string{"Alpha", "ALP"}}},
	}
	inserted, err := SeedFromConfig(db, seeds)
	if err != nil {
		t.Fatalf("SeedFromConfig: %v", err)
	...
	// dedupes to 1 because the UNIQUE silences the second insert. Expect
	// 2 rows total.
	if inserted != 2 {
		t.Errorf("inserted = %d, want 2 (UNIQUE silences canonical dup)", inserted)
	}
}
