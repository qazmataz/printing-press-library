...
// entity_lookups table from the v3 schema.
...
func TestLookup_TableBackedRoundTrip(t *testing.T) {
	...
	got, ok, err = Lookup(db, "team_code", "alpha") // case-insensitive
	if err != nil {
...
func TestUpsert_DifferentAliasesForSameCanonical(t *testing.T) {
	t.Parallel()
	db := openTestDB(t)
...
func TestUpsert_ValidationErrors(t *testing.T) {
	...
	cases := []struct {
		...
		{"empty alias", "team_code", "Alpha", "", "alias is required"},
		{"computed kind", "lowercase", "Alpha", "alpha", "computed kind"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			err := Upsert(db, tc.kind, tc.canonical, tc.alias, "taught")
			if err == nil {
...
func TestSeedBatch_InsertsAndIdempotent(t *testing.T) {
	...
	db := openTestDB(t)
	seeds := []LookupRow{
		{Kind: "team_code", Canonical: "Alpha", Alias: "ALP", Source: "seeded"},
		{Kind: "team_code", Canonical: "Bravo", Alias: "BRV", Source: "seeded"},
		{Kind: "team_long", Canonical: "Alpha", Alias: "Alpha United", Source: "seeded"},
	}
	tx, err := db.Begin()
	if err != nil {
...
func TestSeedBatch_DefaultsSource(t *testing.T) {
	...
	db := openTestDB(t)
	seeds := []LookupRow{
		{Kind: "team_code", Canonical: "Alpha", Alias: "ALP"}, // empty source
	}
	tx, err := db.Begin()
	if err != nil {
		t.Fatalf("begin: %v", err)
