...
Download a pre-built binary for your platform from the [latest release](https://github.com/mvanhorn/printing-press-library/releases/tag/digitalocean-current). On macOS, clear the Gatekeeper quarantine: `xattr -d com.apple.quarantine <binary>`. On Unix, mark it executable: `chmod +x <binary>`.
...
1. Download the `.mcpb` for your platform from the [latest release](https://github.com/mvanhorn/printing-press-library/releases/tag/digitalocean-current).
2. Double-click the `.mcpb` file. Claude Desktop opens and walks you through the install.
3. Fill in `DIGITALOCEAN_BEARER_AUTH` when Claude Desktop prompts you.
...
```json
{
  "mcpServers": {
    "digitalocean": {
      "command": "digitalocean-pp-mcp",
      "env": {
        "DIGITALOCEAN_BEARER_AUTH": "<your-key>"
      }
    }
  }
}
...
```bash
export DIGITALOCEAN_BEARER_AUTH="your-token-here"
```
...
Run `digitalocean-pp-cli --help` for the full command reference and flag list.
...
- **`digitalocean-pp-cli async-invoke create`** - Create
...
- **`digitalocean-pp-cli embeddings create`** - Create
...
- **`digitalocean-pp-cli messages create`** - Create

### models

Manage models

- **`digitalocean-pp-cli models list`** - List
...
- **`digitalocean-pp-cli regions list`** - List
...
- **`digitalocean-pp-cli reports list`** - List
...
- **`digitalocean-pp-cli responses create`** - Create
...
- **`digitalocean-pp-cli sizes list`** - List
...
- **`digitalocean-pp-cli upload-url update`** - Update
...
digitalocean-pp-cli 1-clicks list --json

# Filter to specific fields
digitalocean-pp-cli 1-clicks list --json --select id,name,status

# Dry run — show the request without sending
digitalocean-pp-cli 1-clicks list --dry-run

# Agent mode — JSON + compact + no prompts in one flag
digitalocean-pp-cli 1-clicks list --agent
```
...
Config file: `~/.config/digitalocean-pp-cli/config.toml`
...
| --- | --- | --- | --- |
| `DIGITALOCEAN_BEARER_AUTH` | per_call | Yes | Set to your API credential. |

## Troubleshooting
**Authentication errors (exit code 4)**
- Run `digitalocean-pp-cli doctor` to check credentials
- Verify the environment variable is set: `echo $DIGITALOCEAN_BEARER_AUTH`
