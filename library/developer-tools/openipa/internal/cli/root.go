...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:          "openipa-pp-cli",
		Short:        `Openipa CLI — Il primo CLI per l'Indice delle Pubbliche Amministrazioni — lookup istantaneo di enti, PEC, codici IPA, fatturazione el…`,
		SilenceErrors: true,
		Long: `openipa — il primo CLI per le 22 API web service dell'Indice delle Pubbliche Amministrazioni.

Perché esiste: il portale IPA richiede navigazione manuale. openipa porta tutto sul terminale:
codici destinatario SDI, nodi NSO, PEC ufficiali, strutture ente — in un comando solo.

Comandi chiave non documentati nelle API ufficiali:
  • cf <CF>                  Compliance check parallelo: SFE + NSO + domicilio digitale in un roundtrip
  • fatturazione batch       Legge CF da stdin, restituisce NDJSON con cod_uni_ou pronti per fattura PA
  • enti tree <cod>          Vista Ente → AOO[N] → UO[M] in un comando invece di tre endpoint
  • domicilio verifica <pec> Classifica PEC come attiva / storica / sconosciuta prima di inviare notifiche
  • cerca <email>            Risale all'ente IPA titolare di qualsiasi indirizzo email o PEC

Agent mode: aggiungi --agent per JSON + no-interattivo (CI/pipeline).
Health check: openipa-pp-cli doctor
Ricette: README.md o SKILL.md`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("openipa-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'openipa-pp-cli profile list')")
	...
	rootCmd.AddCommand(domicilioCmd)

	entiCmd := newEntiCmd(flags)
	entiCmd.AddCommand(newEntiTreeCmd(flags))
	rootCmd.AddCommand(entiCmd)

	fatturazioneCmd := newFatturazioneCmd(flags)
	fatturazioneCmd.AddCommand(newFatturazioneBatchCmd(flags))
	rootCmd.AddCommand(fatturazioneCmd)

	rootCmd.AddCommand(newNsoCmd(flags))
	rootCmd.AddCommand(newUoCmd(flags))
	...
	rootCmd.AddCommand(newCercaPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())
	// Transcendence: top-level cf command for compliance check (SFE+NSO+domicilio)
	rootCmd.AddCommand(newCfCmd(flags))
...
func (f *rootFlags) newClient() (*client.Client, error) {
	...
	if cfg.AuthHeader() == "" && !f.dryRun {
		return nil, authErr(fmt.Errorf(
			"AUTH_ID non configurato — le API IPA richiedono un token gratuito.\n" +
				"  Richiedilo su: https://www.indicepa.gov.it/ipa-portale/dati-statistiche/web-service\n" +
				"  Poi esporta: export IPA_auth_id=<il-tuo-codice>\n" +
				"  Oppure salvalo: openipa-pp-cli auth set-token <il-tuo-codice>"))
	}
