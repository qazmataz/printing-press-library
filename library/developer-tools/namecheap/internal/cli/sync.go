...
Exit codes & warnings:
  ...
  namecheap-pp-cli sync

  # Sync specific resources only
  namecheap-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  namecheap-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  namecheap-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  namecheap-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  namecheap-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/namecheap-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "xml_response":
		return db.UpsertXmlResponse(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"xml-response": "/xml.response/ssl/get-list",
	}
