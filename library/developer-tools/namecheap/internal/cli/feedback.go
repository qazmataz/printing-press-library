...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".namecheap-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NAMECHEAP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NAMECHEAP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/developer-tools/namecheap/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.namecheap-pp-cli/feedback.jsonl.
When ` + "`NAMECHEAP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NAMECHEAP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  namecheap-pp-cli feedback list
  namecheap-pp-cli feedback list --limit 5
  namecheap-pp-cli feedback list --json`,
