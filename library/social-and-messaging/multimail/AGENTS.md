...
```bash
multimail-pp-cli doctor --json
multimail-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
multimail-pp-cli which "<capability>" --json
...
```bash
multimail-pp-cli <command> --agent
```
...
```bash
multimail-pp-cli <command> --help
multimail-pp-cli <command> --dry-run --agent
```
