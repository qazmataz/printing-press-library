# Tenderned Printed CLI Agent Guide

This directory is a generated `tenderned-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
tenderned-pp-cli doctor --json
tenderned-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
tenderned-pp-cli which "<capability>" --json
tenderned-pp-cli <command> --help
```
...
```bash
tenderned-pp-cli <command> --agent
```
...
```bash
tenderned-pp-cli <command> --help
tenderned-pp-cli <command> --dry-run --agent
```
