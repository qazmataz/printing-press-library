...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "tenderned-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TENDERNED_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TENDERNED_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "tenderned-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/tenderned-pp-cli/feedback.jsonl.
When ` + "`TENDERNED_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TENDERNED_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  tenderned-pp-cli feedback list
  tenderned-pp-cli feedback list --limit 5
  tenderned-pp-cli feedback list --json`,
