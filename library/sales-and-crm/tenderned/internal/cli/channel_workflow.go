...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  tenderned-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  tenderned-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"buyers", "notices"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  tenderned-pp-cli workflow status

  # Show status as JSON
  tenderned-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("tenderned-pp-cli")
			}
