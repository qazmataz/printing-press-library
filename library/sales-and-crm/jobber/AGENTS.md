# Jobber Printed CLI Agent Guide

This directory is a generated `jobber-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
jobber-pp-cli doctor --json
jobber-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
jobber-pp-cli which "<capability>" --json
...
```bash
jobber-pp-cli <command> --help
jobber-pp-cli <command> --dry-run --agent
```
