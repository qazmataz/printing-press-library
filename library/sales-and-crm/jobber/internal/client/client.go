...
type Client struct {
	...
	// refreshMu serializes RefreshAccessToken so concurrent 401-driven
	// refreshes (sync workers, parallel commands) cannot present the same
	// refresh token twice. Jobber rotates the refresh token on every use;
	// the second concurrent call would post an already-invalidated token
	// and permanently break the rotation chain.
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "jobber-pp-cli", "http")
...
func (c *Client) writeCache(path string, params map[string]string, data json.RawMessage) {
	// Owner-only perms: cache entries embed raw API response bodies that
	// can contain customer rows, contact info, and other tenant data.
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		req.Header.Set("X-JOBBER-GRAPHQL-VERSION", "2025-04-16")
		if c.Config != nil {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "jobber-pp-cli/0.1.0")
		}
...
func (c *Client) doGraphQL(op graphQLOp, method string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	if method != http.MethodGet {
		return nil, 0, fmt.Errorf("GraphQL transport is read-only: refusing %s", method)
	}
	vars, err := op.Variables(params)
	if err != nil {
		return nil, 0, err
	}
	...
	targetURL := c.BaseURL
	if targetURL == "" {
		targetURL = "https://api.getjobber.com/api/graphql"
	}
	if c.DryRun {
		return c.dryRun(http.MethodPost, targetURL, op.root, nil, bodyBytes, headerOverrides, authHeader)
	}
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		req.Header.Set("X-JOBBER-GRAPHQL-VERSION", "2025-04-16")
		if c.Config != nil && c.Config.JobberGraphqlVersion != "" {
			req.Header.Set("X-JOBBER-GRAPHQL-VERSION", c.Config.JobberGraphqlVersion)
		}
		if c.Config != nil {
			for k, v := range c.Config.Headers {
				req.Header.Set(k, v)
		...
		resp, err := c.HTTPClient.Do(req)
		if err != nil {
			lastErr = fmt.Errorf("POST %s: %w", targetURL, err)
			continue
		}
		respBody, err := io.ReadAll(resp.Body)
		resp.Body.Close()
		...
		if resp.StatusCode == http.StatusUnauthorized && !refreshed {
			if err := c.refreshIfStale(authHeader); err != nil {
		...
		apiErr := &APIError{Method: http.MethodPost, Path: "/api/graphql", StatusCode: resp.StatusCode, Body: truncateBody(respBody)}
		if resp.StatusCode == 429 && attempt < maxRetries {
			c.limiter.OnRateLimit()
			wait := cliutil.RetryAfter(resp)
			fmt.Fprintf(os.Stderr, "rate limited, waiting %s (attempt %d/%d, rate adjusted to %.1f req/s)\n", wait, attempt+1, maxRetries, c.limiter.Rate())
		...
		if err := json.Unmarshal(respBody, &gqlResp); err != nil {
			return nil, resp.StatusCode, fmt.Errorf("decoding GraphQL response: %w", err)
		}
		if len(gqlResp.Errors) > 0 {
			if graphqlThrottled(gqlResp.Errors) && attempt < maxRetries {
				c.limiter.OnRateLimit()
				wait := time.Duration(3*(attempt+1)) * time.Second
				fmt.Fprintf(os.Stderr, "GraphQL throttled, waiting %s (attempt %d/%d, rate adjusted to %.1f req/s)\n", wait, attempt+1, maxRetries, c.limiter.Rate())
				time.Sleep(wait)
				lastErr = &APIError{Method: http.MethodPost, Path: "/api/graphql", StatusCode: http.StatusTooManyRequests, Body: truncateBody(respBody)}
				continue
			}
			return nil, http.StatusBadGateway, &APIError{Method: http.MethodPost, Path: "/api/graphql", StatusCode: http.StatusBadGateway, Body: truncateBody(respBody)}
		}
		out, err := op.ExtractResponse(gqlResp.Data)
		if err != nil {
			return nil, resp.StatusCode, err
		}
		c.limiter.OnSuccess()
...
func graphqlThrottled(errors []graphQLError) bool {
