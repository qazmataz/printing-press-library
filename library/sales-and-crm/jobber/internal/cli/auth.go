...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Jobber",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  jobber-pp-cli auth setup\n  jobber-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "No setup URL is configured for this CLI; check the API's docs.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export JOBBER_CLIENT_ID=\"<your-token>\"")
			fmt.Fprintln(w, "  export JOBBER_CLIENT_SECRET=\"<your-token>\"")
			fmt.Fprintln(w, "  export JOBBER_CALLBACK_URL=\"<your-token>\"")
			fmt.Fprintln(w, "  export JOBBER_ACCESS_TOKEN=\"<your-token>\"")
			fmt.Fprintln(w, "  export JOBBER_REFRESH_TOKEN=\"<your-token>\"")
			fmt.Fprintln(w, "  export JOBBER_GRAPHQL_VERSION=\"<your-token>\"")
			fmt.Fprintln(w, "  jobber-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  jobber-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export JOBBER_CLIENT_ID=\"your-token-here\"")
				fmt.Fprintln(w, "  export JOBBER_CLIENT_SECRET=\"your-token-here\"")
				fmt.Fprintln(w, "  export JOBBER_CALLBACK_URL=\"your-token-here\"")
				fmt.Fprintln(w, "  export JOBBER_ACCESS_TOKEN=\"your-token-here\"")
				fmt.Fprintln(w, "  export JOBBER_REFRESH_TOKEN=\"your-token-here\"")
				fmt.Fprintln(w, "  export JOBBER_GRAPHQL_VERSION=\"your-token-here\"")
				fmt.Fprintf(w, "  jobber-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  jobber-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  jobber-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("JOBBER_CLIENT_ID") != "" {
				envStillSet = "JOBBER_CLIENT_ID"
			}
			if envStillSet == "" && os.Getenv("JOBBER_CLIENT_SECRET") != "" {
				envStillSet = "JOBBER_CLIENT_SECRET"
			}
			if envStillSet == "" && os.Getenv("JOBBER_CALLBACK_URL") != "" {
				envStillSet = "JOBBER_CALLBACK_URL"
			}
			if envStillSet == "" && os.Getenv("JOBBER_ACCESS_TOKEN") != "" {
				envStillSet = "JOBBER_ACCESS_TOKEN"
			}
			if envStillSet == "" && os.Getenv("JOBBER_REFRESH_TOKEN") != "" {
				envStillSet = "JOBBER_REFRESH_TOKEN"
			}
			if envStillSet == "" && os.Getenv("JOBBER_GRAPHQL_VERSION") != "" {
				envStillSet = "JOBBER_GRAPHQL_VERSION"
			}
