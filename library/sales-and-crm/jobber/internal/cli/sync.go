...
Resource scoping:
  ...
  jobber-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  jobber-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  jobber-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  jobber-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  jobber-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/jobber-pp-cli/data.db)")
...
func extractPaginationFromEnvelope(envelope map[string]json.RawMessage, cursorParam string) (string, bool) {
	...
	if nextCursor == "" {
		paginationWrapperKeys := []string{"response_metadata", "pagination", "meta", "paging", "pageInfo"}
	...
	hasMoreKeys := []string{"has_more", "hasMore", "has_next", "hasNext", "next_page", "hasNextPage"}
	...
	// Relay GraphQL: hasNextPage lives one level deep inside pageInfo.
	if !hasMore {
		if rawPageInfo, ok := envelope["pageInfo"]; ok {
			var pageInfo map[string]json.RawMessage
			if json.Unmarshal(rawPageInfo, &pageInfo) == nil {
				if raw, ok := pageInfo["hasNextPage"]; ok {
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "jobber_jobs":
		return db.UpsertJobberJobs(data)
	case "payment-records":
		return db.UpsertPaymentRecords(data)
	case "properties":
		return db.UpsertProperties(data)
	case "quotes":
		return db.UpsertQuotes(data)
	case "visits":
		return db.UpsertVisits(data)
	default:
