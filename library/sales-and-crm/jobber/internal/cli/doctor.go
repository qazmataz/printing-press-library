...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  jobber-pp-cli doctor
  jobber-pp-cli doctor --json
  jobber-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export JOBBER_CLIENT_ID=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("JOBBER_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "JOBBER_CLIENT_ID")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "JOBBER_CLIENT_ID")
			}
			if os.Getenv("JOBBER_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "JOBBER_CLIENT_SECRET")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "JOBBER_CLIENT_SECRET")
			}
			if os.Getenv("JOBBER_CALLBACK_URL") != "" {
				authEnvSet = append(authEnvSet, "JOBBER_CALLBACK_URL")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "JOBBER_CALLBACK_URL")
			}
			if os.Getenv("JOBBER_ACCESS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "JOBBER_ACCESS_TOKEN")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "JOBBER_ACCESS_TOKEN")
			}
			if os.Getenv("JOBBER_REFRESH_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "JOBBER_REFRESH_TOKEN")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "JOBBER_REFRESH_TOKEN")
			}
			if os.Getenv("JOBBER_GRAPHQL_VERSION") != "" {
				authEnvSet = append(authEnvSet, "JOBBER_GRAPHQL_VERSION")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "JOBBER_GRAPHQL_VERSION")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'jobber-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'jobber-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'jobber-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'jobber-pp-cli sync' to refresh."
	}
	return report
}
