...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".jobber-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("JOBBER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("JOBBER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "jobber-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.jobber-pp-cli/feedback.jsonl.
When ` + "`JOBBER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`JOBBER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  jobber-pp-cli feedback list
  jobber-pp-cli feedback list --limit 5
  jobber-pp-cli feedback list --json`,
