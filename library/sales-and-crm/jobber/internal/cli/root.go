...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "jobber-pp-cli",
		Short: `Jobber CLI — Read-only Jobber CLI for offline analysis — every GraphQL surface synced to SQLite, every relationship queryable, zero …`,
		Long: `Jobber CLI — Read-only Jobber CLI for offline analysis — every GraphQL surface synced to SQLite, every relationship queryable, zero …
...
Health check: run 'jobber-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("jobber-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'jobber-pp-cli profile list')")
	...
	rootCmd.AddCommand(newJobberJobsCmd(flags))
	...
	rootCmd.AddCommand(newARCmd(flags))
	rootCmd.AddCommand(newWorkflowCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newInvoicesGroupCmd(flags))
	rootCmd.AddCommand(newPaymentRecordsPromotedCmd(flags))
	rootCmd.AddCommand(newPropertiesPromotedCmd(flags))
	rootCmd.AddCommand(newQuotesPromotedCmd(flags))
	rootCmd.AddCommand(newSnapshotCmd(flags))
	rootCmd.AddCommand(newVisitsPromotedCmd(flags))
