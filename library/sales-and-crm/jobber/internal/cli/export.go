...
large datasets as it has no memory pressure.`,
  ...
  jobber-pp-cli export <resource> --format jsonl --output data.jsonl

  # Export with limit
  jobber-pp-cli export <resource> --format jsonl --limit 1000

  # Pipe to another tool
  jobber-pp-cli export <resource> --format jsonl | jq '.id'`,
		Args: cobra.MinimumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			validResources := map[string]bool{
				"clients":         true,
				"invoices":        true,
				"payment-records": true,
				"properties":      true,
				"quotes":          true,
				"visits":          true,
			}
