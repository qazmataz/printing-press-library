...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  jobber-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  jobber-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"clients", "invoices", "jobber_jobs", "payment-records", "properties", "quotes", "visits"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  jobber-pp-cli workflow status

  # Show status as JSON
  jobber-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("jobber-pp-cli")
			}
