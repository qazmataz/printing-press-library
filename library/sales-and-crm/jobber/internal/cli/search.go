...
In local mode: searches locally synced data only.`,
  ...
  jobber-pp-cli search "error timeout"

  # Force local search only
  jobber-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  jobber-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  jobber-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'jobber-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
