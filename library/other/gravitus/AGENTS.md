# Gravitus Printed CLI Agent Guide

This directory is a generated `gravitus-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
gravitus-pp-cli doctor --json
gravitus-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
gravitus-pp-cli which "<capability>" --json
gravitus-pp-cli <command> --help
```
...
```bash
gravitus-pp-cli <command> --help
gravitus-pp-cli <command> --dry-run --agent
```
