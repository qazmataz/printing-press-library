...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Gravitus",
...
profile by name when the installed backend supports it.`,
		Example: `  gravitus-pp-cli auth login --chrome
  gravitus-pp-cli auth login --browser
  gravitus-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  gravitus-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  gravitus-pp-cli auth login --browser\n")
				return nil
			}
			...
			if cookieToolSupportsProfiles(tool.name) {
				...
				if err != nil {
					...
					fmt.Fprintf(w, "\n  gravitus-pp-cli auth login --chrome\n")
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  gravitus-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  gravitus-pp-cli auth login --chrome\n")
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  gravitus-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
