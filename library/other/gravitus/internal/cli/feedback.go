...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".gravitus-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GRAVITUS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GRAVITUS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "gravitus-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.gravitus-pp-cli/feedback.jsonl.
When ` + "`GRAVITUS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GRAVITUS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gravitus-pp-cli feedback list
  gravitus-pp-cli feedback list --limit 5
  gravitus-pp-cli feedback list --json`,
