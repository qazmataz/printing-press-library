...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "gravitus-pp-cli",
		Short: `Gravitus CLI — The only CLI that syncs your Gravitus strength data into your training dashboard.`,
		Long: `Gravitus CLI — The only CLI that syncs your Gravitus strength data into your training dashboard.
...
Health check: run 'gravitus-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("gravitus-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'gravitus-pp-cli profile list')")
	...
	// Auth — with password-based login added
	authCmd := newAuthCmd(flags)
	authCmd.AddCommand(newAuthLoginPasswordCmd(flags))
	...
	// Exercises — analytics subcommands alongside the generated HTML fetch
	exercisesCmd := newExercisesGravitusCmd(flags)
	exercisesCmd.AddCommand(newExercisesPromotedCmd(flags)) // keeps raw HTML fetch at 'exercises fetch'
	rootCmd.AddCommand(exercisesCmd)

	rootCmd.AddCommand(newUsersPromotedCmd(flags))

	// Workouts — list and sync-status alongside the generated workout-get
	workoutsCmd := newWorkoutsPromotedCmd(flags)
	workoutsCmd.AddCommand(newWorkoutsListCmd(flags))
	workoutsCmd.AddCommand(newWorkoutsSyncStatusCmd(flags))
	rootCmd.AddCommand(workoutsCmd)

	// New Gravitus-specific commands
	rootCmd.AddCommand(newGravitusSyncCmd(flags))
	rootCmd.AddCommand(newStatsGravitusCmd(flags))
	rootCmd.AddCommand(newExportCmd(flags))
	rootCmd.AddCommand(newSetDashboardCmd(flags))
