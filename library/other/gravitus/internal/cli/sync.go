...
Resource scoping:
  ...
  gravitus-pp-cli sync

  # Sync specific resources only
  gravitus-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  gravitus-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  gravitus-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  gravitus-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  gravitus-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/gravitus-pp-cli/data.db)")
