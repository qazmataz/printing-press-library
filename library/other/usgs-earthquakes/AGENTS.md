# USGS Earthquakes Printed CLI Agent Guide

This directory is a generated `usgs-earthquakes-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
usgs-earthquakes-pp-cli doctor --json
usgs-earthquakes-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
usgs-earthquakes-pp-cli which "<capability>" --json
usgs-earthquakes-pp-cli <command> --help
```
...
```bash
usgs-earthquakes-pp-cli <command> --agent
```
...
```bash
usgs-earthquakes-pp-cli <command> --help
usgs-earthquakes-pp-cli <command> --dry-run --agent
```
