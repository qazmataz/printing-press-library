...
func shellOutToCLI(cliPath func() (string, error), commandPath []string) server.ToolHandlerFunc {
	...
	return func(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
		if lookupErr != nil {
			return mcplib.NewToolResultError(fmt.Sprintf("companion CLI binary not found: %v\nTried sibling lookup, USGS_EARTHQUAKES_CLI_PATH env var, and PATH.", lookupErr)), nil
		}
...
// config file, change the delivery target, or execute arbitrary shell
// commands via a per-command --notify hook — all of which sit outside the
