...
func (e *APIError) Error() string {
	// USGS FDSN echoes the request URL inside the 4xx body with
	// HTML-escaped ampersands (`&amp;`). Decoding named entities
	// (`&amp;`, `&lt;`, `&gt;`, `&quot;`, `&#39;`) here makes the
	// surfaced URL copy/paste-able for users and keeps the rule-based
	// numeric-entity check honest about what the agent sees.
	return fmt.Sprintf("%s %s returned HTTP %d: %s", e.Method, e.Path, e.StatusCode, html.UnescapeString(e.Body))
}
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "usgs-earthquakes-pp-cli", "http")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		req.Header.Set("User-Agent", "usgs-earthquakes-pp-cli/0.1.0 (+https://github.com/mvanhorn/printing-press-library)")
		if c.Config != nil {
