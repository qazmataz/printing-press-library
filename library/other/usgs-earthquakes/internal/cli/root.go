...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "usgs-earthquakes-pp-cli",
		Short: `Every USGS earthquake feed and event query in one terminal, with offline SQLite cache, agent-native output, and a live watch mode.`,
		Long: `Every USGS earthquake feed and event query in one terminal, with offline SQLite cache, agent-native output, and a live watch mode.
...
Highlights (not in the official API docs):
  ...
  • changes   Diff since the last sync: what events appeared, what events had magnitudes/depths/alerts revised, what events were retracted. Tracks USGS solution revisions over time. Returns 'no revisions recorded …
...
Health check: run 'usgs-earthquakes-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("usgs-earthquakes-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'usgs-earthquakes-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCatalogsPromotedCmd(flags))
	rootCmd.AddCommand(newContributorsPromotedCmd(flags))
	rootCmd.AddCommand(newFeedsPromotedCmd(flags))
	rootCmd.AddCommand(newMetadataPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	rootCmd.AddCommand(newRecentCmd(flags))
	rootCmd.AddCommand(newNearCmd(flags))
	rootCmd.AddCommand(newFeedListCmd(flags))
	rootCmd.AddCommand(newWatchCmd(flags))
	rootCmd.AddCommand(newAftershocksCmd(flags))
	rootCmd.AddCommand(newSwarmDetectCmd(flags))
	...
	rootCmd.AddCommand(newDecodeIDCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("usgs-earthquakes-pp-cli %s\n", version)
		},
	}
}
