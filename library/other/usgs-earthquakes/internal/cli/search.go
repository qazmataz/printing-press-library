...
In local mode: searches locally synced data only.`,
  ...
  usgs-earthquakes-pp-cli search "error timeout"

  # Force local search only
  usgs-earthquakes-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  usgs-earthquakes-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  usgs-earthquakes-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /query
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/query", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'usgs-earthquakes-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
