...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  usgs-earthquakes-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  usgs-earthquakes-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"catalogs", "contributors", "events"}
			totalSynced := 0
			...
			// When --json is set, syncResource's per-event NDJSON lines to os.Stdout
			// would pollute the final JSON envelope below. Temporarily redirect
			// os.Stdout to /dev/null for the loop only; restore before the envelope
			// is encoded.
			...
			// Restore stdout before emitting the JSON envelope so callers see it.
			if devNull != nil {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/usgs-earthquakes-pp-cli/data.db)")
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  usgs-earthquakes-pp-cli workflow status

  # Show status as JSON
  usgs-earthquakes-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("usgs-earthquakes-pp-cli")
			}
