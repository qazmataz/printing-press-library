...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".usgs-earthquakes-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("USGS_EARTHQUAKES_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("USGS_EARTHQUAKES_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "usgs-earthquakes-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.usgs-earthquakes-pp-cli/feedback.jsonl.
When ` + "`USGS_EARTHQUAKES_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`USGS_EARTHQUAKES_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  usgs-earthquakes-pp-cli feedback list
  usgs-earthquakes-pp-cli feedback list --limit 5
  usgs-earthquakes-pp-cli feedback list --json`,
